...
function tranfer2(){
    ...
    cp -rf data/oppo/psw/powermonitor_backup/ ${newpath}/powermonitor_backup/
...
#Ziqing.Guo@BSP.Kernel.Stability, 2018/01/13, add for mini dump and full dump swicth
        ...
        if [ x${boot_completed} == x"1" ]; then
            $XKIT dd if=/vendor/firmware/dpAP_full.mbn of=/dev/block/bootdevice/by-name/apdp conv=fsync
        fi
#endif
    fi
...
#Ziqing.Guo@BSP.Kernel.Stability, 2018/01/13, add for mini dump and full dump swicth
        boot_completed=`getprop sys.boot_completed`
        if [ x${boot_completed} == x"1" ]; then
            $XKIT dd if=/vendor/firmware/dpAP_mini.mbn of=/dev/block/bootdevice/by-name/apdp conv=fsync
        fi
#endif
    fi
...
function collectwifidmesg(){
    WIFIDUMPLOGPATH=/data/vendor/tombstones/rfs/modem
    reason_name=`getprop persist.sys.wifi.minidump.reason`
    system/bin/dmesg > ${WIFIDUMPLOGPATH}/${reason_name}.kinfo
    chown system:system ${WIFIDUMPLOGPATH}/${reason_name}.kinfo
}
